...
/* mFRI(mFRI),
    lwr_robot_fri(lwr_robot_fri),
    srv_callback(srv_callback)*/
{
...
void Fri_console::fri_callback(const kuka_fri_bridge::FRI::ConstPtr& msg){
    ...
    if(msg->position.size() == NB_JOINTS){
        position = msg->position;
    }
    if(msg->effort.size() == NB_JOINTS){
        effort = msg->effort;
    }
    if(msg->damping.size() == NB_JOINTS){
        damping = msg->damping;
    }
    if(msg->stiffness.size() == NB_JOINTS){
...
void Fri_console::ConsoleUpdate(){
    ...
    int index = 0;
    if(fri_msg.position.size() == NB_JOINTS){
    ...
    if(fri_msg.effort.size() == NB_JOINTS){
    ...
    if(fri_msg.stiffness.size() == NB_JOINTS){
    ...
    if(fri_msg.damping.size() == NB_JOINTS){
    ...
    switch (mFRI_CTRL) {
    case FRI_CTRL_POSITION/*FastResearchInterface::JOINT_POSITION_CONTROL*/:
        sprintf(static_txt, "Control Mode    :  JOINT POSITION");
        break;
    case FRI_CTRL_JNT_IMP/*FastResearchInterface::JOINT_IMPEDANCE_CONTROL*/:
        sprintf(static_txt, "Control Mode    :  JOINT IMPEDANCE");
        break;
    case FRI_CTRL_CART_IMP/*FastResearchInterface::CART_IMPEDANCE_CONTROL*/:
    ...
    if(fri_msg.IsRobotArmPowerOn && !fri_msg.DoesAnyDriveSignalAnError)
