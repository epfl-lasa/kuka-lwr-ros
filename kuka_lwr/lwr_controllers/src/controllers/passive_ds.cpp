...
namespace controllers {
    ...
    Passive_ds::Passive_ds(ros::NodeHandle& nh, controllers::Change_ctrl_mode& change_ctrl_mode) : Base_controllers(lwr_controllers::CTRL_MODE::CART_PASSIVE_DS),
    ...
    void Passive_ds::update(KDL::Wrench& wrench, KDL::JntArray& tau_cmd, const KDL::Jacobian& J, const KDL::JntArrayAcc& joint_msr_, const KDL::Twist x_msr_vel_, const KDL::Rotation& rot_msr_, const KDL::Vector& p)
    {
        ...
        nullspace_torque << (Eigen::MatrixXd::Identity(7, 7) - J.data.transpose() * J_transpose_pinv) * (-_jointLimitsGain * joint_msr_.q.data - _desiredJointsGain * (joint_msr_.q.data - qd) - _jointVelocitiesGain * joint_msr_.qdot.data);
